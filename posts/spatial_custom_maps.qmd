...
Next read in all files with the extension .shp and merge them using the
SF package. The countries have different levels of granularity, but this
is not an issue when using the bind_rows function.
...
The `merge_province` function
consolidates the provinces by merging them, generating the geometry for
the resulting merged province, and eliminating the provinces that have
been merged to prevent them from appearing as separate entities.
Subsequently, it merges the newly created "province" with the existing
provinces.
...
[Identifying Districts Using Coordinates with R’s SF Package](/posts/spatial_districts_coordinates.html)

[Replicating the HIV Sub-National Estimates Viewer in Tableau: A Step-by-Step Guide](/posts/ART_Coverage_DR_CONGO.html) (how to create maps in Tableau)
